...
\begin{equation}
   F_{50} =  \text{Number of global minimum points},
\label{equa_F8}
\end{equation}

\begin{equation}
   F_{51} =  \text{Number of global maximum points}.
\label{equa_F9}
\end{equation}

### Correlation of Cartesian Contour ($F_{52}$)

|       Correlation is another new feature computed based on the cartesian contour. The measure is calculated as 

\begin{equation}
   F_{52} =  \frac{\sum_{i=0}^{m}(x_i - \overline{\rm x})(y_i - \overline{\rm y})}{\sqrt{\sum_{i=0}^{m} (x_i - \overline{\rm x})^2 (y_i - \overline{\rm y})^2}},
\label{equa_F10}
\end{equation}
